...
import { writeMMRProof, type MMRProof as StreamableMMRProof } from "./Streamable.js";
...
export function serializeMMRProof(proof: { chunks: Array<{ isLeft: boolean; mmrData: { data: Bytes; value: bigint } }> }, blockTime: bigint = 0n): Bytes {
  // Convert to Streamable format and serialize
  const streamableProof: StreamableMMRProof = {
    blockTime: blockTime,
...
function sha3(data: Bytes): Bytes {
 ...
 * => 00 01 00
 */
function encodeMiniNumberZERO(): Bytes {
 ...
 *   raw UTF-8 bytes
 */
function encodeMiniStringUTF8(text: string): Bytes {
 ...
 */
export function mmrLeafExact(script: string): Bytes {
...
// ============================================================================
// MMR Tree Implementation matching Minima's MMR.java and MMRData.java
...
export interface MMRData {
  data: Bytes;    // 32-byte hash
  value: bigint;  // Sum value (always 0n for TreeKeyNode)
}
...
export interface MMREntry {
  row: number;           // Tree level (0 = leaves)
  entryNumber: bigint;   // Position within the row
  mmrData: MMRData;
}
...
export interface MMRProofChunk {
  isLeft: boolean;   // Is this sibling on the left?
  mmrData: MMRData;  // Sibling's data
}
...
export class MMRTree {
  private entries: Map<string, MMREntry> = new Map();
  private leafCount: number = 0;
  private maxRow: number = 0;
  
  private getKey(row: number, entryNumber: bigint): string {
    return `${row}:${entryNumber}`;
  }
  
  private setEntry(row: number, entryNumber: bigint, data: MMRData): MMREntry {
    const entry: MMREntry = { row, entryNumber, mmrData: data };
    this.entries.set(this.getKey(row, entryNumber), entry);
    if (row > this.maxRow) this.maxRow = row;
    return entry;
  }
  
  private getEntry(row: number, entryNumber: bigint): MMREntry | undefined {
  ...
  getRoot(): MMRData | null {
    if (this.leafCount === 0) return null;
 ...
 */
export function calculateProofRoot(leafData: MMRData, proof: MMRProof): Bytes {
 ...
 */
export function verifyMMRProof(leafPubkey: Bytes, proof: MMRProof, expectedRoot: Bytes): boolean {
  const leafData = createMMRDataLeafNode(leafPubkey, 0n);
  const computedRoot = calculateProofRoot(leafData, proof);
  
  if (computedRoot.length !== expectedRoot.length) return false;
  for (let i = 0; i < computedRoot.length; i++) {
    if (computedRoot[i] !== expectedRoot[i]) return false;
  }
  return true;
}
 ...
 */
function readMiniNumber(data: Bytes, offset: number): { value: bigint; bytesRead: number } {
