...
function parseExports(dir: string, pkgName: string): PackageExports {
  ...
  while (i < lines.length) {
    ...
    if (fnMatch) { exports.functions.push(fnMatch[2]); i++; continue }
    ...
    if (clsMatch) { exports.classes.push(clsMatch[1]); i++; continue }
    const ifaceMatch = trimmed.match(/^export interface (\w+)/)
    if (ifaceMatch) { exports.interfaces.push(ifaceMatch[1]); i++; continue }
    const constMatch = trimmed.match(/^export (const|let|var) (\w+)/)
    if (constMatch) { exports.consts.push(constMatch[2]); i++; continue }

    // Handle multi-line export { ... } from '...' blocks
