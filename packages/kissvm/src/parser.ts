...
class Parser {
    ...
    switch (tok.kind) {
      ...
      case 'LET': {
        ...
        if (this.at('LPAREN')) {
          ...
          return { type: 'STORE_TUPLE', index, value, span: this.span(startPos) };
        }
      ...
      case 'IF': {
        ...
        this.eat('THEN');
        const thenBlock = this.parseBlock(['ELSE', 'ELSEIF', 'ENDIF']);
        const elseifBranches: { cond: ASTNode; body: ASTNode[] }[] = [];
        while (this.tryEat('ELSEIF')) {
          const ec = this.parseExpr();
          this.eat('THEN');
          const eb = this.parseBlock(['ELSE', 'ELSEIF', 'ENDIF']);
          elseifBranches.push({ cond: ec, body: eb });
        }
        ...
        this.eat('ENDIF');
        return { type: 'IF', cond, then: thenBlock, elseifBranches, else: elseBlock, span: this.span(startPos) };
      }
      case 'FOR': {
        this.eat();
        const name = this.eat('IDENT').value;
      ...
      case 'WHILE': {
        ...
        this.eat('ENDWHILE');
        return { type: 'WHILE', cond, body, span: this.span(startPos) };
      }
      case 'FUNC': {
        this.eat();
        const name = this.eat('IDENT').value;
      ...
      case 'EXEC': {
        ...
        const scriptExpr = this.parseExpr();
        return { type: 'EXEC', script: scriptExpr, span: this.span(startPos) };
      }
      case 'MAST': {
        this.eat();
        if (this.at('LBRACE')) {
  ...
  private parseNor(): ASTNode {
    ...
    while (this.at('NOR')) {
      ...
      left = { type: 'BINARY', op: 'NOR', left, right };
    }
    return left;
  }
  ...
  private parseNand(): ASTNode {
    ...
    while (this.at('NAND')) {
      ...
      left = { type: 'BINARY', op: 'NAND', left, right };
    }
    return left;
  }
  ...
  private parseXor(): ASTNode {
    let left = this.parseNxor();
    while (this.at('XOR')) {
      this.eat();
      const right = this.parseNxor();
      left = { type: 'BINARY', op: 'XOR', left, right };
    }
    return left;
  }

  private parseNxor(): ASTNode {
    let left = this.parseComparison();
    while (this.at('NXOR')) {
      this.eat();
      const right = this.parseComparison();
      left = { type: 'BINARY', op: 'NXOR', left, right };
    }
    return left;
  }
  ...
  private parseBitOr(): ASTNode {
    ...
    while (this.at('BITOR')) {
      ...
      left = { type: 'BINARY', op: 'BITOR', left, right };
    }
    return left;
  }
  ...
  private parseBitXor(): ASTNode {
    ...
    while (this.at('BITXOR')) {
      this.eat();
      const right = this.parseBitAnd();
      left = { type: 'BINARY', op: 'BITXOR', left, right };
    }
    return left;
  }
  ...
  private parseBitAnd(): ASTNode {
    ...
    while (this.at('BITAND')) {
      this.eat();
      const right = this.parseShift();
      left = { type: 'BINARY', op: 'BITAND', left, right };
    }
    return left;
  }

  /** Bit-shift operators: lower precedence than bitwise, higher than arithmetic */
  ...
  private parseUnary(): ASTNode {
    ...
    if (this.at('BITNOT')) {
      this.eat();
      return { type: 'UNARY', op: 'BITNOT', expr: this.parsePrimary() };
    }
  ...
  parsePrimary(): ASTNode {
    ...
    switch (tok.kind) {
      ...
      case 'CHECKSIG': {
        ...
        if (this.at('LPAREN')) {
          this.eat('LPAREN');
          const ckArgs = this.parseSpaceArgList('RPAREN');
          this.eat('RPAREN');
          return { type: 'CHECKSIG', args: ckArgs, span: this.span(startPos) };
        }
        return { type: 'CHECKSIG', args: [], span: this.span(startPos) };
      }
      case 'SHA3':
      case 'SHA2':
      case 'HASH': {
