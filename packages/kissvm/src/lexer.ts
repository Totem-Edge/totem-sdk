...
export function tokenize(source: string): Token[] {
  ...
  while (i < source.length) {
    ...
    if (ch === '&') { tokens.push({ kind: 'BITAND', value: '&', pos: start }); i++; continue; }
    if (ch === '|') { tokens.push({ kind: 'BITOR',  value: '|', pos: start }); i++; continue; }
    if (ch === '^') { tokens.push({ kind: 'BITXOR', value: '^', pos: start }); i++; continue; }
    if (ch === '~') { tokens.push({ kind: 'BITNOT', value: '~', pos: start }); i++; continue; }
