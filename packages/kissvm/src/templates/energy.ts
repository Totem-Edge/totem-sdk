...
export function buildRecTradingScript(
  ...
  return {
    ...
    script: [
      ...
      ``,
      `// RFC-016 I4: exhaustive selection — an unknown (issuance, retirement)`,
      `// combination must fail closed, not fall through to RETURN TRUE.`,
      ...
      `ELSEIF STATE(${options.retirementPort}) EQ 1 THEN`,
      `  ASSERT SIGNEDBY(registry)`,
      `  ASSERT STATE(${options.issuancePort}) EQ 0`,
      `ELSEIF STATE(${options.issuancePort}) EQ 0 AND STATE(${options.retirementPort}) EQ 0 THEN`,
      `  ASSERT SIGNEDBY(registry)`,
      `ELSE`,
      `  RETURN FALSE`,
      `ENDIF`,
