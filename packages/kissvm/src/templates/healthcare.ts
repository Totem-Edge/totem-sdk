...
export function buildMedicalDeviceRegulationScript(
  ...
  return {
    ...
    script: [
      ...
      // RFC-016 P4: advance the counter (was a vacuous `pms GTE 0`).
      `ASSERT STATE(${options.postMarketPort}) EQ INC(pms)`,
      ``,
      `// Adverse event reporting`,
      `LET events = PREVSTATE(${options.adverseEventPort})`,
      `ASSERT INC(events) LTE 9999`,
      `ASSERT STATE(${options.adverseEventPort}) EQ INC(events)`,
      ``,
      `RETURN TRUE`,
    ].join('\n'),
...
export function buildClinicalTrialScript(
  ...
  return {
    ...
    script: [
      ...
      // RFC-016 P4: enrolling advances the counter and enforces the target.
      `ASSERT INC(enrolled) LTE ${options.enrollmentTarget}`,
      `ASSERT STATE(${options.enrollmentPort}) EQ INC(enrolled)`,
      ``,
      `// Blinding integrity`,
      ...
      // RFC-016 P4: bound and advance the safety-report counter.
      `ASSERT INC(safetyEvents) LTE 9999`,
      `ASSERT STATE(${options.safetyReportPort}) EQ INC(safetyEvents)`,
      ``,
      `RETURN TRUE`,
    ].join('\n'),
