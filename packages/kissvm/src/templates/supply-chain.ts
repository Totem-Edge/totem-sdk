...
export function buildBillOfLadingScript(
  ...
  return {
    ...
    script: [
      ...
      ``,
      `// RFC-016 I4: exhaustive custody selection — an unknown custody value`,
      `// must fail closed, not fall through to RETURN TRUE.`,
      ...
      `ELSEIF STATE(${options.custodyPort}) EQ 0x${options.carrierPkd} THEN`,
      `  ASSERT SIGNEDBY(carrier)`,
      `  ASSERT @BLOCK LTE ${options.deliveryDeadlineBlock}`,
      `ELSEIF STATE(${options.custodyPort}) EQ 0x${options.consigneePkd} THEN`,
