...
export function buildSmartContractExecutionScript(
  ...
  lines.push(
    ``,
    `// RFC-016 I4: exhaustive outcome selection — performance=0 AND breach=0`,
    `// (or any unknown combination) fails closed.`,
    `IF STATE(${options.performancePort}) EQ 1 AND STATE(${options.breachPort}) EQ 0 THEN`,
    `  ASSERT SIGNEDBY(partyA) AND SIGNEDBY(partyB)`,
    `ELSEIF STATE(${options.breachPort}) EQ 1 THEN`,
