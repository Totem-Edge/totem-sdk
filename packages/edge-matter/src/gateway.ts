/**
 * Matter Edge Gateway — wires a MatterTransportPort into an EdgeRuntime.
 */
...
export interface MatterGatewayConfig {
  ...
  vendorId: number;
  productId: number;
  /** Attributes to subscribe to on start. */
  ...
  return {
    ...
    async start(): Promise<void> {
      ...
      unsubCommissioned = config.transport.onCommissioned((node) => {
        if (config.runtime.ports.proof) {
          config.runtime.ports.proof.createProof({
            ...
            claims: [{ nodeId: node.nodeId, vendorId: node.vendorId, productId: node.productId, vendorName: node.vendorName, productName: node.productName, timestamp: Date.now() }],
          }).catch(() => {});
        }
      });
      ...
      for (const binding of config.subscriptions ?? []) {
        const sub = await config.transport.subscribe(binding.nodeId, binding.endpointId, binding.clusterId, binding.attributeIds, binding.minInterval, binding.maxInterval);
        sub.onChange((reports) => {
          for (const report of reports) {
            if (binding.sensorId && config.runtime.ports.proof) {
              config.runtime.ports.proof.createProof({
                subject: `sensor:${binding.sensorId}`,
                claims: [{ sensorId: binding.sensorId, nodeId: report.nodeId, endpointId: report.endpointId, clusterId: report.clusterId, attributeId: report.attributeId, value: report.value, dataType: report.dataType, timestamp: report.receivedAt }],
              }).catch(() => {});
            }
          }
        });
    ...
    async commission(discriminator, setupCode) {
      try {
        const node = await config.transport.commission({ discriminator, vendorId: config.vendorId, productId: config.productId }, setupCode);
