 ...
 *
 * This one-way derivation creates a cryptographic ancestor that is:
 *   - Distinct from baseSeed (cannot be brute-forced back to it)
 *   - Deterministic (same baseSeed always produces the same root)
 *   - The parent of all unified child address seeds
 *
 * The root TreeKey derived from this seed is the identity anchor and is NEVER
 * used as a spend address on-chain.
 *
 * @param baseSeed - 32-byte wallet base seed (from mnemonic)
 * @returns 32-byte root private seed
 */
export function deriveRootPrivSeed(baseSeed) {
    const baseSeedSerialized = serializeMiniData(baseSeed);
 ...
 *
 * This mirrors the Minima Wallet.java hashObjects pattern but uses root_priv_seed
 * as the first operand instead of baseSeed directly, making each child
 * cryptographically descended from the root rather than a sibling of it.
 *
 * @param baseSeed - 32-byte wallet base seed (from mnemonic)
 * @param index - Address index (0-63)
 * @returns 32-byte child seed for the TreeKey at this address
 */
export function deriveUnifiedChildSeed(baseSeed, index) {
...
export function precomputeTransactionCoinID(inputs, outputs) {
    ...
    if (baseCoinId.length < 2) {
        throw new Error(`Cannot precompute output coinIDs: input[0].coinId is ${baseCoinId.length} byte(s) ` +
