...
export function createBleGateway(config: BleGatewayConfig): BleGateway {
  ...
  return {
    ...
    async read(peripheralId, serviceUuid, characteristicUuid) {
      try { const value = await config.transport.read(peripheralId, serviceUuid, characteristicUuid); return { ok: true, data: { value } }; }
      catch (e) { return { ok: false, error: String(e) }; }
    },

    async write(peripheralId, serviceUuid, characteristicUuid, data) {
      try { await config.transport.write(peripheralId, serviceUuid, characteristicUuid, data); return { ok: true }; }
      catch (e) { return { ok: false, error: String(e) }; }
    },

    async subscribe(peripheralId, serviceUuid, characteristicUuid) {
      try { await config.transport.subscribe(peripheralId, serviceUuid, characteristicUuid); return { ok: true }; }
      catch (e) { return { ok: false, error: String(e) }; }
    },
  };
}
