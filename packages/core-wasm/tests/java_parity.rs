...
fn test_java_golden_vectors() {
    ...
    let vectors_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../core/test-vectors.json");
    ...
    assert_eq!(
        test_vectors.wots_params.w, WOTS_W as u32,
        "WOTS w parameter mismatch: Java={}, Rust={}",
        test_vectors.wots_params.w, WOTS_W
    );
    assert_eq!(
        test_vectors.wots_params.n,
        (WOTS_N * 8) as u32,
    ...
    assert_eq!(
        test_vectors.wots_params.total_chains, WOTS_L as u32,
        "WOTS L parameter mismatch: Java={}, Rust={}",
        test_vectors.wots_params.total_chains, WOTS_L
    );
    ...
    for (i, vector) in test_vectors.vectors.iter().enumerate() {
        ...
        let expected_digest = hex::decode(&vector.digest_hex).expect("Invalid digest hex");
        ...
        let public_key_full =
            hex::decode(&vector.public_key_full_hex).expect("Invalid public key hex");
        ...
        let digest_bytes = hex::decode(&vector.digest_hex).expect("Invalid digest hex");
        ...
        if vector.should_verify {
            ...
            let expected_pk_digest =
                hex::decode(&vector.public_key_digest_hex).expect("Invalid pk digest hex");
            ...
            assert_eq!(
                ...
                i + 1,
                hex::encode(&computed_pk_digest),
                vector.public_key_digest_hex
            );
