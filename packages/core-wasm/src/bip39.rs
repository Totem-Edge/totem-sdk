...
pub static WORD_LIST: [&str; 2049] = [
    ...
    "desk", "despair", "destroy", "detail", "detect", "develop", "device", "devote", "diagram",
    ...
    "fatal", "father", "fatigue", "fault", "favorite", "feature", "february", "federal", "fee",
    "feed", "feel", "female", "fence", "festival", "fetch", "fever", "few", "fiber", "fiction",
    "field", "figure", "file", "film", "filter", "final", "find", "fine", "finger", "finish",
    "fire", "firm", "first", "fiscal", "fish", "fit", "fitness", "fix", "flag", "flame", "flash",
    ...
    "hotel", "hour", "hover", "hub", "huge", "human", "humble", "humor", "hundred", "hunt",
    "hurdle", "hurry", "hurt", "husband", "hybrid", "ice", "icon", "idea", "identify", "idle",
    "ignore", "ill", "illegal", "illness", "image", "imitate", "immense", "immune", "impact",
    ...
    "kid", "kidney", "kind", "kingdom", "kiss", "kit", "kitchen", "kite", "kitten", "kiwi", "knee",
...
///
/// Minima-compatible: uses SHA3-256 of the phrase bytes directly,
/// without PBKDF2 or checksum validation.
...
/// Clean and normalize a seed phrase using prefix matching.
///
/// Matches Minima's BIP39 implementation: each word is matched by
/// its first 4 characters against the BIP39 word list.
pub fn clean_seed_phrase(phrase: &str) -> String {
    ...
    for word in words {
        ...
        if let Some(full_word) = WORD_LIST.iter().find(|w| w.starts_with(prefix)) {
            cleaned.push(*full_word);
        } else {
...
///
/// Checks that all words are in the BIP39 word list and the phrase
/// has exactly 24 words.
pub fn validate_phrase(phrase: &str) -> bool {
    ...
    words.iter().all(|word| {
        let prefix = if word.len() >= 4 { &word[..4] } else { word };
        WORD_LIST.iter().any(|w| w.starts_with(prefix))
    })
}
...
pub fn generate_word_list() -> String {
    ...
    // Convert entropy to 24 word indices (11 bits each, 24 × 11 = 264 bits from 256)
    ...
    // Use remaining bits for the last word if needed
    while words.len() < 24 {
...
mod tests {
    ...
    #[test]
    fn test_validate_phrase_invalid_length() {
        assert!(!validate_phrase("abandon abandon abandon"));
    }
