  ...
  const poll = async () => {
    ...
    try {
      ...
      if (data.result && !data.error) {
        ...
        }).catch((err) => console.warn('[Background] TX_CONFIRMED sendMessage failed:', err));
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === 'install') {
    ...
    console.log('[Update] Previous version:', details.previousVersion);
    // Preserve user data on update — no storage clear.
...
chrome.runtime.onSuspend.addListener(() => {
  console.log('[Background] Service worker suspending — cleaning up connections');
  initSessions.clear();
  cachedPrice = null;
});
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'totem-address-generation-keepalive') {
    ...
    if (portfolioStreamManager.isCurrentlyStreaming()) {
      portfolioStreamManager.forceRefresh().catch((err) => console.warn('[Background] forceRefresh failed:', err));
    }
  }
});
    ...
    case 'WOTS_SEND': {
      ...
      try {
        ...
        try {
          ...
          chrome.runtime.sendMessage({ type: 'TX_STAGE_UPDATE', stage: 'mining' }).catch((err) => console.warn('[Background] TX_STAGE_UPDATE sendMessage failed:', err));
          ...
          const { serializeTxBody, mineTxPoW } = await import('@totemsdk/txpow');
          ...
          if (portfolioStreamManager.isCurrentlyStreaming()) {
            setTimeout(() => portfolioStreamManager.forceRefresh().catch((err) => console.warn('[Background] forceRefresh failed:', err)), 5_000);
            setTimeout(() => portfolioStreamManager.forceRefresh().catch((err) => console.warn('[Background] forceRefresh failed:', err)), 12_000);
          }

          // Return early — skip the txnimport path
          return {
            ok: true,
            txpowid: minedFinalizeResult.txpowid,
            leaseId: minedFinalizeResult.leaseId,
                ...
                try {
                  ...
                  if (tabOrigin === reqOrigin) {
                    chrome.tabs.sendMessage(tab.id, {
                      ...
                      data: []
                    }).catch((err) => console.warn('[Background] accountsChanged sendMessage failed:', err));
                  }
                } catch {}
              }
            }
