...
export class WalletManager {
  ...
  async restoreSession(): Promise<'active' | 'expired' | 'locked'> {
    ...
    if (wasActive) {
      ...
      }).catch((err) => console.warn('[Session] sessionExpired sendMessage failed:', err));
  ...
  lock(): void {
    ...
    chrome.storage.session.set({ lastActivity: 0 }).catch((err) => console.warn('[Session] clearing lastActivity failed:', err));
    
    // Clear session flag from session storage
    this.setSessionActive(false).catch((err) => console.warn('[Session] setSessionActive(false) failed:', err));
  }
  ...
  async signTransaction(params: any) {
    ...
    try {
      const { CORE_BUILD_ID } = await import('@totemsdk/core');
