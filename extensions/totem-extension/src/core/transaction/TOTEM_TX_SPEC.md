...
**Canonical Source:** `packages/core/src/minimaWireSerializer.ts`
...
**Source:** `packages/core/src/minimaWireSerializer.ts`
...
**Key dependency:** Uses `mxToHex()` from `packages/core/src/minima32.ts` for Mx→Hex conversion.
...
|-----------|-----------------|
| Primitive serialization | `packages/core/src/minimaWireSerializer.ts` |
| TreeKey signing | `packages/core/src/treekey.ts` |
