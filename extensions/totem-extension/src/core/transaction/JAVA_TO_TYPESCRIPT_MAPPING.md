...
|-----------|-----------------|--------|-------|
| `Winternitz_*.java` | `packages/core/src/wots.js` | ✅ Full | GMSSRandom PRNG, 34-chain signatures, BouncyCastle-compatible |
| `TreeKey_*.java` | `packages/core/src/treekey.js` | ✅ Full | 3-level hierarchy (64×64×64), parent-child caching |
| `TreeKeyNode_*.java` | `packages/core/src/treekey.js` | ✅ Full | Embedded in TreeKey, childCache for performance |
| `GMSSRandom_bouncycastle.java` | `packages/core/src/wots.js` | ✅ Full | `gmssRandom()` function, byte-exact with BouncyCastle |
| `WinternitzOTSignature_bouncycastle.java` | `packages/core/src/wots.js` | ✅ Full | Reference for signing algorithm |
| `WinternitzOTSVerify_bouncycastle.java` | `packages/core/src/wots.js` | ✅ Full | Reference for verification algorithm |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `MMR_*.java` | `packages/core/src/mmr.ts` | ✅ Full | MMRTree class, proof generation |
| `MMRData_*.java` | `packages/core/src/mmr.ts` | ✅ Full | `createMMRDataLeafNode()`, `combineMMRData()` |
| `MMREntry_*.java` | `packages/core/src/mmr.ts` | ✅ Full | MMREntry type with data, row, isLeft fields |
| `MMREntryNumber_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeMMREntryNumber()` - compact format |
| `MMRProof_*.java` | `packages/core/src/mmr.ts` | ✅ Full | `MMRProof` type, serialization in minimaWireSerializer.ts |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `MiniData_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeMiniData()` - 4-byte BE length + bytes |
| `MiniNumber_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeMiniNumber()` - scale-44 BigDecimal |
| `MiniString_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeMiniString()` - 4-byte len + UTF-8 |
| `MiniByte_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeMiniByte()` - single byte |
| `Streamable_*.java` | `packages/core/src/javaStreamables.ts`, `packages/totem-extension/src/core/transaction/utils/Streamable.ts` | ✅ Full | Interface pattern for wire format |
| `FastByteArrayStream_*.java` | N/A | ❌ N/A | Java I/O utility, not needed in TypeScript |
| `BaseConverter_*.java` | `packages/core/src/minima32.ts` | ✅ Full | Minima Base32 encoding |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `Signature_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeSignature()` - rootPubkey + proofs[] |
| `SignatureProof_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeSignatureProof()` - leafPubkey (32B digest) + sig (1088B) + MMRProof |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `Transaction_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeTransaction()` |
...
| `Coin_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeCoin()` - all 12 fields |
| `CoinProof_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeCoinProof()` - Coin + MMRProof |
| `StateVariable_*.java` | `packages/core/src/minimaWireSerializer.ts` | ⚡ Partial | Simplified format, full type-aware pending |
| `Token_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | Token metadata serialization |
| `ScriptProof_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | `serializeScriptProof()` |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `Address_*.java` | `packages/core/src/derive.ts` | ✅ Full | `deriveAddress()` - SHA3(script) |
| `ADDRESS_*.java` | `packages/core/src/script.ts` | ✅ Full | ADDRESS opcode implementation |
| `CHECKSIG_*.java` | `packages/core/src/script.ts` | ✅ Full | CHECKSIG opcode for signature verification |
| `CHECKSIGTests_*.java` | `packages/core/test/` | 🔧 Test Only | Test vectors |
...
| `BIP39_*.java` | `packages/core/src/bip39.ts` | ✅ Full | Mnemonic generation/validation |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `sign_*.java` | `packages/core/src/treekey.js` | ✅ Full | `setUses()` + `sign()` (3-proof chain matching Java TreeKey.sign()) |
...
| `txnexport_*.java` | `packages/core/src/minimaWireSerializer.ts` | ✅ Full | Export = serialization |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `Crypto_*.java` | `packages/core/src/wots.js`, `packages/core/src/javaStreamables.ts` | ✅ Full | SHA3-256 via @noble/hashes, used throughout serialization |
...
| Java File | TypeScript File | Status | Notes |
|-----------|-----------------|--------|-------|
| `AddressTests_*.java` | `packages/core/src/address.test.ts` | 🔧 Test Only | Address derivation vectors |
| `CoinTests_*.java` | `packages/core/src/minimaWireSerializer.test.ts` | 🔧 Test Only | Coin serialization vectors |
| `TokenTests_*.java` | `packages/core/src/minimaWireSerializer.test.ts` | 🔧 Test Only | Token serialization vectors |
...
| File | Purpose |
|------|---------|
| `packages/core/src/wots.js` | WOTS signatures, GMSSRandom, Winternitz |
| `packages/core/src/treekey.js` | TreeKey hierarchy, signing methods |
| `packages/core/src/mmr.ts` | MMR tree construction and proofs |
| `packages/core/src/minimaWireSerializer.ts` | All Streamable serialization |
| `packages/core/src/javaStreamables.ts` | Java-compatible type definitions |
| `packages/core/src/derive.ts` | Address derivation |
| `packages/core/src/bip39.ts` | BIP39 mnemonic handling |
| `packages/core/src/minima32.ts` | Minima Base32 encoding |
| `packages/core/src/script.ts` | Script opcodes |
