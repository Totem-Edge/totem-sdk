...
**Status:** In progress — **P0 landed** (audit checked in; invariant helpers + static detector; README unaudited notice). **P1 landed**: the confirmed criticals are repaired — `statechain` reclaim anchors to `PREVSTATE` with owner continuity; `authority.ActionAuthorization` requires a fixed authority + monotonic nonce; `agent-policy.PaymentIntent` binds the committed amount/recipient to the actual output (`VERIFYOUT`) and a fixed signer; `proof` delegation uses a committed delegator + root authority; `identity` delegation requires the configured delegator; `provider-bond` heartbeat requires the configured probe signer; `txpow` is renamed to `buildAttestedTxPoWMetaScript` with a fixed attestor (deprecated alias kept); the fail-open branches in `energy` REC, `supply-chain` BoL and `legal` smart-contract are now exhaustive `ELSE RETURN FALSE`; empty policies/ledger chains throw at construction. A refined detector asserts the repaired templates satisfy the invariants (15 tests). Remaining: P2 (MAST root convergence + canonical witness `ScriptProof`s), P3 (anchor/root-rotation/action transaction binding, delegation proof verification + expiry units), P4 (high-severity families incl. industrial-action escrow/reveal, temporal vesting/decay, liquidity-bond/provider-bond economics, manifest/layers), P5 (tx-builder multisig validity/digest, fund-tx rename, negative target).
**Created:** 2026-09-26
