...
| AUD-016 | P1 | Default personal lease certificate verification accepts arbitrary nonempty signatures | FIXED | `packages/wots-lease/src/certificate.ts`. `certificateSignatureVerified` fails closed without a real verifier; `PersonalLeaseNodeConfig.verify` added; test flipped to expect rejection. |
...
| AUD-018 | P1 | Authority usage limits stop applying after the first window | FIXED | Rust/WASM mirror `packages/authority/rust/src/usage.rs` now matches TS (shortcut removed, window anchored to `now`, exclusive end); cargo tests added. |
