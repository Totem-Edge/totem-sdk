# Totem Core Yellow Paper

**The cryptographic foundation of the Totem SDK — WOTS+ signatures, TreeKey hierarchy, MMR proofs, and byte-exact Minima serialization, implemented in both TypeScript and Rust/WASM.**

**Version:** 3.1
**Last Updated:** 2026-07-27
**Status:** Active

This document describes the Winternitz One-Time Signature (WOTS) implementation and cryptographic architecture that underpins the entire Totem SDK. The core is implemented in two complementary forms: a **TypeScript reference implementation** (`@totemsdk/core`) for Node.js, browser, and extension environments, and a **Rust/WASM engine** (`@totemsdk/core-wasm`) for deterministic, high-performance execution with zero native dependencies.
...
## WOTS Lease & Watermark System

### Why Leases and Watermarks Exist

WOTS keys are one-time-use. Reusing a key to sign two different messages reveals enough information to forge signatures — the security guarantee collapses completely. Every transaction requires a fresh, never-before-used signing index.

In a multi-device or multi-signer environment, two parties could independently try to use the same index. Without coordination, both signatures leak information and the key is compromised.

**Leases** are time-bounded reservations of specific signing indices. When a client calls `PREPARE`, the server finds the next unused index, reserves it exclusively (row-level lock in PostgreSQL), returns a signed JWT token proving the reservation, and sets a TTL. If the client doesn't finalize within the TTL, the lease expires.

**Watermarks** track the highest index that has been allocated or used. They never decrease (monotonic advancement). Even if a lease expires without being finalized, the watermark ensures that index is never re-allocated. The client maintains a local watermark and validates it against the server.

### Per-Address Watermark Tracking

Each of the 64 addresses tracks its own `(l1, l2)` watermark independently:
...
The flat index for any signing slot is: `flatIndex = addressIndex × 4096 + l1 × 64 + l2` (range: 0 to 262,143).

### Lease Lifecycle

```
PREPARE → LEASED → SIGN → FINALIZE → USED
                  ↓
              EXPIRED (TTL exceeded, index freed for re-allocation)
                  ↓
              CANCELLED (explicit release)
```

**Prepare:** Client requests signing indices. Server allocates next available `(addressIndex, l1, l2)`, creates a lease with TTL, returns `leaseId` + `leaseToken`.

**Sign:** Client signs the transaction locally using the reserved indices. Private keys never leave the client.

**Finalize:** Client submits the signed transaction. Server validates the lease token, verifies the signature, marks the lease as `USED`, and advances the watermark.

**Expiry:** If the client doesn't finalize within the TTL, the lease expires. The watermark is NOT rolled back — the index is permanently consumed to prevent reuse.

### Security Invariants

1. **Watermarks never decrease.** `watermark(t₂) >= watermark(t₁)` for all `t₂ > t₁`. Violation constitutes critical security failure.
2. **One signature per index.** No `(addressIndex, l1, l2)` tuple may ever be used to sign more than one message.
3. **Lease tokens are cryptographically bound.** The JWT includes the reserved indices, the client's root public key, and an expiry timestamp. Tampering is detectable.
4. **Row-level locking prevents races.** PostgreSQL `SELECT ... FOR UPDATE` ensures two concurrent PREPARE calls cannot receive the same index.
5. **Client-side validation.** The client maintains a local watermark and validates it against the server before signing. Mismatch indicates a potential compromise.

### Crash Recovery

If a client crashes after PREPARE but before FINALIZE:
- The lease remains in `LEASED` status on the server
- On restart, the client queries active leases and either finalizes or cancels them
- The watermark has already advanced past the reserved index — it cannot be reused
- If the lease expires, the index is permanently consumed (watermark does not roll back)

### Multisig Coordination

In multisig scenarios, the `MultisigManager` coordinates leases across multiple signers:
- Each signer independently calls PREPARE and receives their own lease
- The coordinator collects all signatures
- Each signer independently calls FINALIZE
- If any signer fails to finalize, their lease expires and the transaction must be re-prepared with fresh indices
